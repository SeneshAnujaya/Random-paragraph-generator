...
const paraResult = document.querySelector(".paragraphs");
...
generateBtn.addEventListener("click", function () {
  ...
  if (isNaN(amountVal) || amountVal < 0 || amountVal > 9) {
    paraResult.innerHTML = text[random];
  } else {
    let textArr = text.slice(0, amountVal);

    let result = textArr.map(function (para) {
      return `<p class="paragraph">${para}</p>`;
    });

    paraResult.innerHTML = result.join("");
